...
- **Cause**: SSN was stored as plain text in the database (`users` table schema, `signup` mutation).
- **Fix**: Added AES-256-GCM encryption via `encryptSSN()` in `lib/crypto.ts`; store only encrypted SSN and last 4 digits for display.
...
- **Cause**: Used `dangerouslySetInnerHTML` to render transaction descriptions in `TransactionList` component.
...
- **Cause**: Returned fake account with $100 balance when DB fetch failed after insert in `createAccount` mutation.
...
- **Cause**: Used loop with float division causing precision errors in `fundAccount` mutation.
...
- **Cause**: DOB field only checked for required, no age or future date validation in signup form and `signup` mutation.
- **Fix**: Added client and server validation via `validateDOB()` requiring user to be 18+ and DOB not in the future.
...
- **Cause**: Only checked card prefix (4 or 5), no checksum validation in `FundingModal` component.
- **Fix**: Implemented Luhn algorithm via `isValidCardNumber()` in `lib/validation.ts`; accept 13-19 digit cards.
...
- **Cause**: Password validation only checked length and one number in signup form and `signup` mutation.
- **Fix**: Require uppercase, lowercase, number, and special character via `validatePassword()` on both client and server.
...
- **Cause**: Query missing `.all()` call to fetch all transaction records in `getTransactions` query.
...
- **Cause**: Account numbers generated with `Math.random()` which is predictable in `generateAccountNumber()`.
...
- **Cause**: Weak email regex and no typo detection; lowercase conversion without user notice in signup form.
- **Fix**: Improved regex pattern, added `detectEmailTypo()` for common TLD typos (.con, .cm), added lowercase hint.
...
- **Cause**: Client min validation set to 0.0 instead of 0.01 in `FundingModal` component.
...
- **Cause**: Server schema had routing number as optional regardless of transfer type in `fundAccount` mutation.
- **Fix**: Added Zod refinement to require 9-digit routing number with ABA checksum when funding type is "bank".
...
- **Cause**: Only accepted Visa (4) and Mastercard (5) prefixes, rejecting valid Amex, Discover, etc.
- **Fix**: Added `getCardType()` in `lib/validation.ts` to detect Visa, Mastercard, Amex, Discover, Diners, and JCB cards.
...
- **Cause**: `initDb()` created new DB connection each call, stored in array but never closed.
...
- **Cause**: Login created new sessions without invalidating existing ones, allowing unlimited concurrent sessions per user.
- **Fix**: Delete all existing sessions for user before creating new one (single session policy) in both `signup` and `login` mutations.
...
- **Cause**: Sessions expiring soon only logged a warning without taking action, causing abrupt logouts for active users in `createContext()`.
...
- **Cause**: N+1 query problem in `getTransactions` - fetched same account from DB for every transaction in a loop.
...
- **Cause**: Logout always returned `success: true` without verifying session deletion; only attempted deletion when `ctx.user` existed in `logout` mutation.
...
- **Cause**: Validation only checked for 2 uppercase letters, accepting invalid codes like "XX" in signup form and `signup` mutation.
- **Fix**: Validate against `VALID_STATES` (50 US states + DC + 5 territories) imported from `lib/validation.ts` on both client and server.
...
- **Cause**: Client only accepted US 10-digit format; server pattern was inconsistent and too loose.
- **Fix**: Aligned both to `isValidPhoneNumber()` pattern `/^(\d{10}|\+\d{11,15})$/` - accepts US (10 digits) or international (+11-15 digits).
...
- **Cause**: Amount regex allowed multiple leading zeros like "00100" in `FundingModal` component.
- **Fix**: Changed pattern to `isValidAmount()` using `/^(0|[1-9]\d*)(\.\d{1,2})?$/` which only allows single zero or numbers starting with 1-9.
...
- **Cause**: `getTransactions` query had no `orderBy` clause, returning transactions in unpredictable order.
...
|------|-------|-----------|----------|
| Unit | 74 | Vitest | `tests/unit/` |
| Integration | 9 | Vitest + SQLite | `tests/integration/` |
| E2E | 34 | Playwright | `tests/e2e/` |
| **Total** | **117** | | |
...
| VAL-205 | E2E | `tests/e2e/dashboard.spec.ts` | 1 |
| VAL-206 | Unit | `tests/unit/validation.test.ts:188` | 7 |
| VAL-206 | E2E | `tests/e2e/dashboard.spec.ts` | 1 |
| VAL-207 | Unit | `tests/unit/validation.test.ts:219` | 4 |
| VAL-207 | E2E | `tests/e2e/dashboard.spec.ts` | 2 |
...
| VAL-209 | E2E | `tests/e2e/dashboard.spec.ts` | 1 |
| VAL-210 | Unit | `tests/unit/validation.test.ts:278` | 7 |
| VAL-210 | E2E | `tests/e2e/dashboard.spec.ts` | 2 |
| PERF-404 | Integration | `tests/integration/account.test.ts:66` | 1 |
| PERF-404 | E2E | `tests/e2e/dashboard.spec.ts` | 1 |
| PERF-405 | Integration | `tests/integration/account.test.ts:85` | 1 |
| PERF-405 | E2E | `tests/e2e/dashboard.spec.ts` | 1 |
...
└── e2e/
    ...
    ├── signup.spec.ts    # Signup validation tests
    └── dashboard.spec.ts # Funding & transaction tests
```
...
The following items are intentionally left as-is for this interview context but would need to be addressed:

### Security

1. **Hardcoded Fallback Secrets**: `SSN_ENCRYPTION_KEY` and `JWT_SECRET` have fallback values (`"temporary-secret-for-interview"`) in code. In production, these should throw errors if environment variables are not set.

2. **Static Salt for Key Derivation**: The `scryptSync` call in `lib/crypto.ts` uses a literal `"salt"` string. Production should use a unique random salt per encryption or store as an environment variable.

### Financial Accuracy

3. **Float-Based Balance Storage**: Account balances are stored as `REAL` (float) in SQLite. For production banking, consider storing amounts as integers (cents) or using a decimal library like `decimal.js` or `bignumber.js`.
